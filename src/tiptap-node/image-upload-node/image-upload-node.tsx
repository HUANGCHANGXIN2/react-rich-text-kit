...
import { zhCNLabels, type RichTextEditorLabels } from "@/labels"
...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    if (file.size > options.maxSize) {
      const error = new Error(options.labels.uploadTooLarge(options.maxSize))
    ...
    try {
      if (!options.upload) {
        throw new Error(options.labels.uploadNotConfigured)
      }
      ...
      if (!url) throw new Error(options.labels.uploadNoUrl)
      ...
      if (!abortController.signal.aborted) {
        ...
        options.onError?.(
          error instanceof Error ? error : new Error(options.labels.uploadFailed)
        )
      }
      return null
    }
  ...
  const uploadFiles = async (files: File[]): Promise<string[]> => {
    if (!files || files.length === 0) {
      options.onError?.(new Error(options.labels.uploadNoFile))
      return []
    }
    ...
    if (options.limit && files.length > options.limit) {
      options.onError?.(
        new Error(
          options.labels.uploadTooMany(options.limit)
        )
      )
      return []
    }
      ...
      <div className="tiptap-image-upload-preview-content">
        ...
        <div className="tiptap-image-upload-actions">
          ...
          <Button
            type="button"
            variant="ghost"
            aria-label={labels.imageRemove}
            title={labels.imageRemove}
            onClick={(e) => {
              e.stopPropagation()
              onRemove()
            }}
...
const DropZoneContent: React.FC<{
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      <span className="tiptap-image-upload-text">
        {labels.imageUploadDrop}
      </span>
      <span className="tiptap-image-upload-subtext">
        {labels.imageUploadHint(limit, maxSize)}
      </span>
    </div>
  </>
)
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  const labels = extension.options.labels ?? zhCNLabels
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0) {
      extension.options.onError?.(new Error(labels.uploadNoFile))
      return
    }
    ...
    >
      {!hasFiles && (
        <ImageUploadDragArea onFile={handleUpload}>
          <DropZoneContent maxSize={maxSize} limit={limit} labels={labels} />
        </ImageUploadDragArea>
      )}
        ...
        <div className="tiptap-image-upload-previews">
          {fileItems.length > 1 && (
            <div className="tiptap-image-upload-header">
              <span>{labels.imageUploading(fileItems.length)}</span>
              <Button
                type="button"
                variant="ghost"
                onClick={(e) => {
